<div>
                        ...
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            ...
                            <td class="py-2 px-4 text-center flex justify-center">
                                <div class="flex justify-center space-x-2">
                                    ...
                                    <button type="button"
                                        class="bg-green-500 text-white text-xs px-4 py-2 rounded hover:bg-green-600"
                                        wire:click="confirm({{ $application->application_id }})">Confirm</button>
                                    <button type="button"
                                        class="bg-red-500 text-white text-xs px-4 py-2 rounded hover:bg-red-600"
                                        wire:click="reject({{ $application->application_id }})">Decline</button>
                                </div>
                            </td>
                        </tr>
                    @endif
    ...
    <div id="encode-control-number-table" class="hidden">
        <table class="w-full border-collapse bg-white">
            ...
            <tbody class="text-gray-600 text-sm font-light">
                @foreach ($applications as $application)
                    @if ($application->status == 'Confirm' && $application->control_number == null)
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            ...
                            <td class="py-2 px-4 text-center">{{ $application->job->job }}</td>
                            <td class="py-2 px-4 text-center">{{ $application->job->country->country }}</td>
                            <td class="py-3 px-6 text-center">{{ $application->status }}</td>
                            <td class="py-3 px-6 text-center">
                                @if ($application->control_number == null)
                                    <button type="button"
                                        class="bg-green-500 text-white text-2xl font-bold px-3 py-1 rounded hover:bg-green-600"
                                        wire:click="encodeControlNumber({{ $application->application_id }})">+</button>
                                @else
                                    {{ $application->control_number ?? 'Control Number' }}
                                @endif
                            </td>
